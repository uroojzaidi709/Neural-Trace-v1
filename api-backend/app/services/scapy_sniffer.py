...
"""
Neural-Trace Scapy Packet Sniffer (Production Ready Fix)
======================================================
"""

import asyncio
import httpx
import statistics  # Top level import for thread safety
from collections import defaultdict
...
flow_tracker   = defaultdict(list)
seen_flow_keys = set()  # Tracks first-seen direction
main_loop      = None   # To hold references to the main FastAPI event loop
...
def packet_callback(packet):
    try:
        ...
        # FIX: Simulation friendly packet threshold (Changed from 50 to 3)
        if len(pkts) >= 3:
            features = extract_features(pkts)
            if features and main_loop:
                # FIX: Thread-safe communication to submit task back to the main async thread
                asyncio.run_coroutine_threadsafe(
                    classify_and_ingest(key, features, src_ip, dst_port), 
                    main_loop
                )
            del flow_tracker[key]
            seen_flow_keys.discard(key)
        ...
        print(f"[SCAPY] Packet Callback Error: {e}")


async def scapy_background_task():
    global main_loop
    print("[SCAPY] Packet sniffer task initialized")
    ...
    # Capture the main async event loop reference
    main_loop = asyncio.get_event_loop()
    
    # Run Scapy sniff in background thread safely
